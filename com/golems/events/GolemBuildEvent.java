...
public class GolemBuildEvent extends Event 
{
	...
	/** 
	 * @return true if theGolem should not be
	 * spawned (even if it is not null).
	 **/
	...
	protected boolean getAreGolemBlocksSameMeta() 
	{
		...
		state = this.isGolemXAligned ? this.worldObj.getBlockState(armsX[0]) : this.worldObj.getBlockState(armsZ[0]);
		int metaArm1 = this.blockBelow.getMetaFromState(state);
		state = this.isGolemXAligned ? this.worldObj.getBlockState(armsX[1]) : this.worldObj.getBlockState(armsZ[1]);
